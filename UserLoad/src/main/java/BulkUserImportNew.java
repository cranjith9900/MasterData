...
public class BulkUserImportNew {

    private static Properties config = new Properties();
    private static final int BATCH_SIZE = 1000;

    static {
        try (InputStream input = BulkUserImportNew.class.getClassLoader().getResourceAsStream("config.properties")) {
            if (input == null) {
    ...
    private static JSONObject createJsonObjectFromCsvLine(String[] userData) throws JSONException {
        ...
        putIfNotEmpty(jsonObject, "firstName", userData[2]);
        putIfNotEmpty(jsonObject, "lastName", userData[3]);
        ...
        //putIfNotEmpty(jsonObject, "siteErpId", userData[11]);
        //putIfNotEmpty(jsonObject, "costCenterErpId", userData[6]);
        
        if (userData[4] != null && !userData[4].trim().isEmpty()) {
            JSONArray groupsArray = new JSONArray();
            String[] groups = userData[4].split(";"); // Split by comma
        ...
        jsonObject.put("active", userData[5]);
    ...
    private static boolean sendPostRequest(JSONArray jsonArray) {
        try {
            URL url = new URL(config.getProperty("apiUrl"));
